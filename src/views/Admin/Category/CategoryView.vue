...
import DialogViewVue from '@/components/Admin/Category/DialogView.vue';
...
const { fetchProducts, products, query, searchProducts } = useCategory()
...
const deleteProductById = async (id) => {
  ...
  const data = await categoryServiceApi._delete(id)
  // console.log(data)
  if (data.success) {
    loadData()
          ...
          <v-table density="compact" hover>
            ...
            <tbody>
              <tr v-if="products.length>0" v-for="(item, index) in products" :key="index">
                <td style="width: 250px;height: 58px;"><b><p style="width: 100%;max-height: 58px;overflow: hidden;display: -webkit-box;-webkit-box-orient: vertical;-webkit-line-clamp: 1;">{{ index+1 }}</p></b></td>
